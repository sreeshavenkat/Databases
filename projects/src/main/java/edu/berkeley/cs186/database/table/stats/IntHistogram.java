...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    float floatDistinct = (float) this.getNumDistinct();
    float floatMin = (float) (value.getInt() - getMinValue());
    float floatMax = (float) (getMaxValue() - value.getInt());
    float floatMaxMin = (float) (getMaxValue() - getMinValue());
    switch (predicate) {
      case NOT_EQUALS:
        reductionFactor = (float) 1 - ((float) 1 / floatDistinct);
        break;
      case LESS_THAN:
        reductionFactor = floatMin / floatMaxMin;
        break;
      case LESS_THAN_EQUALS:
        reductionFactor = (floatMin / floatMaxMin) + (1 / floatDistinct);
        break;
      case EQUALS:
        reductionFactor = (float) 1 / floatDistinct;
        break;
      case GREATER_THAN_EQUALS:
        reductionFactor = floatMax / floatMaxMin + (float) 1 / floatDistinct;
        break;
      case GREATER_THAN:
        reductionFactor = floatMax / floatMaxMin;
        break;
    }
